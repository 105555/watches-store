<template>
  <div>
    ...
    <v-container>
      <v-row>
        <v-col>
          <v-text-field prepend-inner-icon="mdi-magnify" single-line clearable hide-details label="搜尋商品..."
    ...
    <v-container>
      <v-row>
        <v-col v-for="item in displayItem" :key="item.id" cols="12" sm="6" md="4" class="mx-auto text-center">
            ...
            <div class="d-flex justify-space-between align-items-center">
              <div>
                <router-link :to="{ name: 'ShopItem', params: { id: item.id } }" class="d-flex flex-column">
                  ...
                  <v-card-subtitle class="pb-7 mt-3 pl-6 text-l" style="color:#000">定價: ${{ item.prices
                    }}</v-card-subtitle>
                </router-link>
              </div>
      ...
      <br />
      <v-pagination v-model="page" :length="Math.ceil(totalItem / maxitem)" class="pb-10"></v-pagination>
    </v-container>
  </div>
</template>
